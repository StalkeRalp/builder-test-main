        ...
        function applyDashboardUrlFilters() {
            ...
            if (status && statusFilter && Array.from(statusFilter.options).some((o) => o.value === status)) {
                statusFilter.value = status;
            }
            if (type && typeFilter && Array.from(typeFilter.options).some((o) => o.value === type)) {
                typeFilter.value = type;
            }
            if (client && clientFilter && Array.from(clientFilter.options).some((o) => o.value === client)) {
        ...
        function bindGlobalNavigationSearch() {
            ...
            searchInput.addEventListener('keydown', (event) => {
                ...
                navigateFromQuery(searchInput.value || '');
            });
        }
        ...
        function bindKeyboardShortcuts() {
            window.addEventListener('keydown', (event) => {
                ...
                const routes = {
                    '1': 'index.html',
                    '2': 'create-project.html',
                    '3': 'clients.html',
                    '4': 'tickets.html?status=open',
                    '5': 'messages.html?unread=1',
                    '6': 'calendar.html',
                    '7': 'reports.html'
                };
        ...
        function navigateFromQuery(rawQuery) {
            ...
            if (q.includes('message') || q.includes('chat')) {
                window.location.href = `messages.html?q=${encodeURIComponent(query)}`;
                return;
            }
            ...
            if (q.includes('profil') || q.includes('profile')) {
                window.location.href = 'profile.html';
                return;
            }
            if (q.includes('nouveau') || q.includes('create')) {
        ...
        window.addEventListener('beforeunload', () => {
            try {
                ...
                console.error('index unsubscribe error:', error);
            }
        });
